    ...
    _power_up((get_rx_id() == 0x0024 && get_tx_id() == 0x0028) ? POWER_IO : 0)
{
    ...
    this->get_rx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&rfx_xcvr::get_locked, this, dboard_iface::UNIT_RX));
    BOOST_FOREACH(const std::string &name, _rx_gain_ranges.keys()){
        this->get_rx_subtree()->create<double>("gains/"+name+"/value")
            .coerce(std::bind(&rfx_xcvr::set_rx_gain, this, std::placeholders::_1, name))
            .set(_rx_gain_ranges[name].start());
        this->get_rx_subtree()->create<meta_range_t>("gains/"+name+"/range")
            .set(_rx_gain_ranges[name]);
    }
    this->get_rx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&rfx_xcvr::set_lo_freq, this, dboard_iface::UNIT_RX, std::placeholders::_1))
        .set((_freq_range.start() + _freq_range.stop())/2.0);
    this->get_rx_subtree()->create<meta_range_t>("freq/range").set(_freq_range);
    this->get_rx_subtree()->create<std::string>("antenna/value")
        .subscribe(std::bind(&rfx_xcvr::set_rx_ant, this, std::placeholders::_1))
        .set("RX2");
    ...
    this->get_tx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&rfx_xcvr::get_locked, this, dboard_iface::UNIT_TX));
    this->get_tx_subtree()->create<int>("gains"); //phony property so this dir exists
    this->get_tx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&rfx_xcvr::set_lo_freq, this, dboard_iface::UNIT_TX, std::placeholders::_1))
        .set((_freq_range.start() + _freq_range.stop())/2.0);
    this->get_tx_subtree()->create<meta_range_t>("freq/range").set(_freq_range);
    this->get_tx_subtree()->create<std::string>("antenna/value")
        .subscribe(std::bind(&rfx_xcvr::set_tx_ant, this, std::placeholders::_1)).set(rfx_tx_antennas.at(0));
