...
sbx_xcvr::sbx_xcvr(ctor_args_t args) : xcvr_dboard_base(args){
    ...
    this->get_rx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&sbx_xcvr::get_locked, this, dboard_iface::UNIT_RX));
    BOOST_FOREACH(const std::string &name, sbx_rx_gain_ranges.keys()){
        this->get_rx_subtree()->create<double>("gains/"+name+"/value")
            .coerce(std::bind(&sbx_xcvr::set_rx_gain, this, std::placeholders::_1, name))
            .set(sbx_rx_gain_ranges[name].start());
        this->get_rx_subtree()->create<meta_range_t>("gains/"+name+"/range")
            .set(sbx_rx_gain_ranges[name]);
    }
    this->get_rx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&sbx_xcvr::set_lo_freq, this, dboard_iface::UNIT_RX, std::placeholders::_1))
        .set((freq_range.start() + freq_range.stop())/2.0);
    this->get_rx_subtree()->create<meta_range_t>("freq/range").set(freq_range);
    this->get_rx_subtree()->create<std::string>("antenna/value")
        .subscribe(std::bind(&sbx_xcvr::set_rx_ant, this, std::placeholders::_1))
        .set("RX2");
    ...
    this->get_tx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&sbx_xcvr::get_locked, this, dboard_iface::UNIT_TX));
    BOOST_FOREACH(const std::string &name, sbx_tx_gain_ranges.keys()){
        this->get_tx_subtree()->create<double>("gains/"+name+"/value")
            .coerce(std::bind(&sbx_xcvr::set_tx_gain, this, std::placeholders::_1, name))
    ...
    this->get_tx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&sbx_xcvr::set_lo_freq, this, dboard_iface::UNIT_TX, std::placeholders::_1))
        .set((freq_range.start() + freq_range.stop())/2.0);
    this->get_tx_subtree()->create<meta_range_t>("freq/range").set(freq_range);
    this->get_tx_subtree()->create<std::string>("antenna/value")
        .subscribe(std::bind(&sbx_xcvr::set_tx_ant, this, std::placeholders::_1))
