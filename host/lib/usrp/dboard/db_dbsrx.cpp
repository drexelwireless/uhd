...
dbsrx::dbsrx(ctor_args_t args) : rx_dboard_base(args){
    ...
    this->get_rx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&dbsrx::get_locked, this));
    BOOST_FOREACH(const std::string &name, dbsrx_gain_ranges.keys()){
        this->get_rx_subtree()->create<double>("gains/"+name+"/value")
            .coerce(std::bind(&dbsrx::set_gain, this, std::placeholders::_1, name))
            .set(dbsrx_gain_ranges[name].start());
        this->get_rx_subtree()->create<meta_range_t>("gains/"+name+"/range")
            .set(dbsrx_gain_ranges[name]);
    }
    this->get_rx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&dbsrx::set_lo_freq, this, std::placeholders::_1));
    ...
    this->get_rx_subtree()->create<double>("bandwidth/value")
        .coerce(std::bind(&dbsrx::set_bandwidth, this, std::placeholders::_1));
