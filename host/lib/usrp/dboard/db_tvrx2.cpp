...
tvrx2::tvrx2(ctor_args_t args) : rx_dboard_base(args){
    ...
    this->get_rx_subtree()->create<sensor_value_t>("sensors/lo_locked")
        .publish(std::bind(&tvrx2::get_locked, this));
    this->get_rx_subtree()->create<sensor_value_t>("sensors/rssi")
        .publish(std::bind(&tvrx2::get_rssi, this));
    this->get_rx_subtree()->create<sensor_value_t>("sensors/temperature")
        .publish(std::bind(&tvrx2::get_temp, this));
    BOOST_FOREACH(const std::string &name, tvrx2_gain_ranges.keys()){
        this->get_rx_subtree()->create<double>("gains/"+name+"/value")
            .coerce(std::bind(&tvrx2::set_gain, this, std::placeholders::_1, name));
        this->get_rx_subtree()->create<meta_range_t>("gains/"+name+"/range")
            .set(tvrx2_gain_ranges[name]);
    }
    this->get_rx_subtree()->create<double>("freq/value")
        .coerce(std::bind(&tvrx2::set_lo_freq, this, std::placeholders::_1));
    ...
    this->get_rx_subtree()->create<bool>("enabled")
        .coerce(std::bind(&tvrx2::set_enabled, this, std::placeholders::_1))
        .set(_enabled);
    this->get_rx_subtree()->create<bool>("use_lo_offset")
        .set(false);
    this->get_rx_subtree()->create<double>("bandwidth/value")
        .coerce(std::bind(&tvrx2::set_bandwidth, this, std::placeholders::_1))
        .set(_bandwidth);
