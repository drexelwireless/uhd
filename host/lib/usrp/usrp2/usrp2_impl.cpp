    ...
    device_addr(_device_addr)
{
    ...
    for (size_t mbi = 0; mbi < device_args.size(); mbi++){
        ...
        _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
            ...
            .subscribe(std::bind(&usrp2_impl::set_mb_eeprom, this, mb, std::placeholders::_1));
        ...
        _tree->create<double>(mb_path / "tick_rate")
            .publish(std::bind(&usrp2_clock_ctrl::get_master_clock_rate, _mbc[mb].clock))
            .subscribe(std::bind(&usrp2_impl::update_tick_rate, this, std::placeholders::_1));
        ...
        case usrp2_iface::USRP_N210_R4:{
            ...
            _tree->create<double>(rx_codec_path / "gains/digital/value")
                .subscribe(std::bind(&usrp2_codec_ctrl::set_rx_digital_gain, _mbc[mb].codec, std::placeholders::_1)).set(0);
            ...
            _tree->create<double>(rx_codec_path / "gains/fine/value")
                .subscribe(std::bind(&usrp2_codec_ctrl::set_rx_digital_fine_gain, _mbc[mb].codec, std::placeholders::_1)).set(0);
        }break;
            ...
            if (_mbc[mb].gps and _mbc[mb].gps->gps_detected())
            {
                BOOST_FOREACH(const std::string &name, _mbc[mb].gps->get_sensors())
                {
                    _tree->create<sensor_value_t>(mb_path / "sensors" / name)
                        .publish(std::bind(&gps_ctrl::get_sensor, _mbc[mb].gps, name));
                }
            }
            else
            {
        ...
        _tree->create<sensor_value_t>(mb_path / "sensors/mimo_locked")
            .publish(std::bind(&usrp2_impl::get_mimo_locked, this, mb));
        _tree->create<sensor_value_t>(mb_path / "sensors/ref_locked")
            .publish(std::bind(&usrp2_impl::get_ref_locked, this, mb));
        ...
        _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
            .subscribe(std::bind(&usrp2_impl::update_rx_subdev_spec, this, mb, std::placeholders::_1));
        _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
            .subscribe(std::bind(&usrp2_impl::update_tx_subdev_spec, this, mb, std::placeholders::_1));

        const fs_path rx_fe_path = mb_path / "rx_frontends" / "A";
        const fs_path tx_fe_path = mb_path / "tx_frontends" / "A";

        _tree->create<std::complex<double> >(rx_fe_path / "dc_offset" / "value")
            .coerce(std::bind(&rx_frontend_core_200::set_dc_offset, _mbc[mb].rx_fe, std::placeholders::_1))
            .set(std::complex<double>(0.0, 0.0));
        _tree->create<bool>(rx_fe_path / "dc_offset" / "enable")
            .subscribe(std::bind(&rx_frontend_core_200::set_dc_offset_auto, _mbc[mb].rx_fe, std::placeholders::_1))
            .set(true);
        _tree->create<std::complex<double> >(rx_fe_path / "iq_balance" / "value")
            .subscribe(std::bind(&rx_frontend_core_200::set_iq_balance, _mbc[mb].rx_fe, std::placeholders::_1))
            .set(std::complex<double>(0.0, 0.0));
        _tree->create<std::complex<double> >(tx_fe_path / "dc_offset" / "value")
            .coerce(std::bind(&tx_frontend_core_200::set_dc_offset, _mbc[mb].tx_fe, std::placeholders::_1))
            .set(std::complex<double>(0.0, 0.0));
        _tree->create<std::complex<double> >(tx_fe_path / "iq_balance" / "value")
            .subscribe(std::bind(&tx_frontend_core_200::set_iq_balance, _mbc[mb].tx_fe, std::placeholders::_1))
            .set(std::complex<double>(0.0, 0.0));
        ...
        for (size_t dspno = 0; dspno < _mbc[mb].rx_dsps.size(); dspno++){
            ...
            _tree->access<double>(mb_path / "tick_rate")
                .subscribe(std::bind(&rx_dsp_core_200::set_tick_rate, _mbc[mb].rx_dsps[dspno], std::placeholders::_1));
            fs_path rx_dsp_path = mb_path / str(boost::format("rx_dsps/%u") % dspno);
            _tree->create<meta_range_t>(rx_dsp_path / "rate/range")
                .publish(std::bind(&rx_dsp_core_200::get_host_rates, _mbc[mb].rx_dsps[dspno]));
            _tree->create<double>(rx_dsp_path / "rate/value")
                .set(1e6) //some default
                .coerce(std::bind(&rx_dsp_core_200::set_host_rate, _mbc[mb].rx_dsps[dspno], std::placeholders::_1))
                .subscribe(std::bind(&usrp2_impl::update_rx_samp_rate, this, mb, dspno, std::placeholders::_1));
            _tree->create<double>(rx_dsp_path / "freq/value")
                .coerce(std::bind(&rx_dsp_core_200::set_freq, _mbc[mb].rx_dsps[dspno], std::placeholders::_1));
            _tree->create<meta_range_t>(rx_dsp_path / "freq/range")
                .publish(std::bind(&rx_dsp_core_200::get_freq_range, _mbc[mb].rx_dsps[dspno]));
            _tree->create<stream_cmd_t>(rx_dsp_path / "stream_cmd")
                .subscribe(std::bind(&rx_dsp_core_200::issue_stream_command, _mbc[mb].rx_dsps[dspno], std::placeholders::_1));
        }
        ...
        _tree->access<double>(mb_path / "tick_rate")
            .subscribe(std::bind(&tx_dsp_core_200::set_tick_rate, _mbc[mb].tx_dsp, std::placeholders::_1));
        _tree->create<meta_range_t>(mb_path / "tx_dsps/0/rate/range")
            .publish(std::bind(&tx_dsp_core_200::get_host_rates, _mbc[mb].tx_dsp));
        _tree->create<double>(mb_path / "tx_dsps/0/rate/value")
            .set(1e6) //some default
            .coerce(std::bind(&tx_dsp_core_200::set_host_rate, _mbc[mb].tx_dsp, std::placeholders::_1))
            .subscribe(std::bind(&usrp2_impl::update_tx_samp_rate, this, mb, 0, std::placeholders::_1));
        _tree->create<double>(mb_path / "tx_dsps/0/freq/value")
            .coerce(std::bind(&tx_dsp_core_200::set_freq, _mbc[mb].tx_dsp, std::placeholders::_1));
        _tree->create<meta_range_t>(mb_path / "tx_dsps/0/freq/range")
            .publish(std::bind(&tx_dsp_core_200::get_freq_range, _mbc[mb].tx_dsp));
        ...
        _tree->access<double>(mb_path / "tick_rate")
            .subscribe(std::bind(&time64_core_200::set_tick_rate, _mbc[mb].time64, std::placeholders::_1));
        _tree->create<time_spec_t>(mb_path / "time/now")
            .publish(std::bind(&time64_core_200::get_time_now, _mbc[mb].time64))
            .subscribe(std::bind(&time64_core_200::set_time_now, _mbc[mb].time64, std::placeholders::_1));
        _tree->create<time_spec_t>(mb_path / "time/pps")
            .publish(std::bind(&time64_core_200::get_time_last_pps, _mbc[mb].time64))
            .subscribe(std::bind(&time64_core_200::set_time_next_pps, _mbc[mb].time64, std::placeholders::_1));
        //setup time source props
        _tree->create<std::string>(mb_path / "time_source/value")
            .subscribe(std::bind(&time64_core_200::set_time_source, _mbc[mb].time64, std::placeholders::_1))
            .set("none");
        _tree->create<std::vector<std::string> >(mb_path / "time_source/options")
            .publish(std::bind(&time64_core_200::get_time_sources, _mbc[mb].time64));
        //setup reference source props
        _tree->create<std::string>(mb_path / "clock_source/value")
            .subscribe(std::bind(&usrp2_impl::update_clock_source, this, mb, std::placeholders::_1))
            .set("internal");
        ...
        case usrp2_iface::USRP_N210_R4:
            _tree->create<time_spec_t>(mb_path / "time/cmd")
                .subscribe(std::bind(&usrp2_fifo_ctrl::set_time, _mbc[mb].fifo_ctrl, std::placeholders::_1));
        default: break; //otherwise, do not register
        }
        _tree->access<double>(mb_path / "tick_rate")
            .subscribe(std::bind(&usrp2_fifo_ctrl::set_tick_rate, _mbc[mb].fifo_ctrl, std::placeholders::_1));
        ...
        _tree->create<user_settings_core_200::user_reg_t>(mb_path / "user/regs")
            .subscribe(std::bind(&user_settings_core_200::set_reg, _mbc[mb].user, std::placeholders::_1));
        ...
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/rx_eeprom")
            .set(rx_db_eeprom)
            .subscribe(std::bind(&usrp2_impl::set_db_eeprom, this, mb, "rx", std::placeholders::_1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/tx_eeprom")
            .set(tx_db_eeprom)
            .subscribe(std::bind(&usrp2_impl::set_db_eeprom, this, mb, "tx", std::placeholders::_1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/gdb_eeprom")
            .set(gdb_eeprom)
            .subscribe(std::bind(&usrp2_impl::set_db_eeprom, this, mb, "gdb", std::placeholders::_1));
        ...
        BOOST_FOREACH(const std::string &name, _tree->list(db_tx_fe_path)){
            _tree->access<double>(db_tx_fe_path / name / "freq" / "value")
                .subscribe(std::bind(&usrp2_impl::set_tx_fe_corrections, this, mb, std::placeholders::_1));
        }
        const fs_path db_rx_fe_path = mb_path / "dboards" / "A" / "rx_frontends";
        BOOST_FOREACH(const std::string &name, _tree->list(db_rx_fe_path)){
            _tree->access<double>(db_rx_fe_path / name / "freq" / "value")
                .subscribe(std::bind(&usrp2_impl::set_rx_fe_corrections, this, mb, std::placeholders::_1));
        }
    }
