    ...
    _tick_rate(0.0) // Forces a clock initialization at startup
{
    ...
    _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
        .set(mb_eeprom)
        .subscribe(std::bind(&b200_impl::set_mb_eeprom, this, std::placeholders::_1));
    ...
    _tree->create<double>(mb_path / "tick_rate")
        .coerce(std::bind(&b200_impl::set_tick_rate, this, std::placeholders::_1))
        .publish(std::bind(&b200_impl::get_tick_rate, this))
        .subscribe(std::bind(&b200_impl::update_tick_rate, this, std::placeholders::_1));
    ...
    _tree->create<sensor_value_t>(mb_path / "sensors" / "ref_locked")
        .publish(std::bind(&b200_impl::get_ref_locked, this));
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
        .coerce(std::bind(&b200_impl::coerce_subdev_spec, this, std::placeholders::_1))
        .set(subdev_spec_t())
        .subscribe(std::bind(&b200_impl::update_subdev_spec, this, "rx", std::placeholders::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
        .coerce(std::bind(&b200_impl::coerce_subdev_spec, this, std::placeholders::_1))
        .set(subdev_spec_t())
        .subscribe(std::bind(&b200_impl::update_subdev_spec, this, "tx", std::placeholders::_1));
    ...
    _tree->create<time_spec_t>(mb_path / "time" / "now")
        .publish(std::bind(&time_core_3000::get_time_now, _radio_perifs[0].time64))
        .subscribe(std::bind(&b200_impl::set_time, this, std::placeholders::_1))
        .set(0.0);
    //re-sync the times when the tick rate changes
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&b200_impl::sync_times, this));
    _tree->create<time_spec_t>(mb_path / "time" / "pps")
        .publish(std::bind(&time_core_3000::get_time_last_pps, _radio_perifs[0].time64));
    BOOST_FOREACH(radio_perifs_t &perif, _radio_perifs)
    {
        _tree->access<time_spec_t>(mb_path / "time" / "pps")
            .subscribe(std::bind(&time_core_3000::set_time_next_pps, perif.time64, std::placeholders::_1));
    }
    ...
    _tree->create<std::string>(mb_path / "time_source" / "value")
        .coerce(std::bind(&check_option_valid, "time source", time_sources, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_time_source, this, std::placeholders::_1));
    ...
    _tree->create<std::string>(mb_path / "clock_source" / "value")
        .coerce(std::bind(&check_option_valid, "clock source", clock_sources, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_clock_source, this, std::placeholders::_1));
    ...
    BOOST_FOREACH(const gpio_attr_map_t::value_type attr, gpio_attr_map)
    {
            ...
            .set(0)
            .subscribe(std::bind(&b200_impl::set_fp_gpio, this, _radio_perifs[0].fp_gpio, attr.first, std::placeholders::_1));
    }
    _tree->create<boost::uint32_t>(mb_path / "gpio" / "FP0" / "READBACK")
        .publish(std::bind(&b200_impl::get_fp_gpio, this, _radio_perifs[0].fp_gpio));
...
void b200_impl::setup_radio(const size_t dspno)
{
    ...
    _tree->access<time_spec_t>(mb_path / "time" / "cmd")
        .subscribe(std::bind(&radio_ctrl_core_3000::set_time, perif.ctrl, std::placeholders::_1));
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&radio_ctrl_core_3000::set_tick_rate, perif.ctrl, std::placeholders::_1));
    ...
    _tree->access<double>(rx_dsp_path / "rate" / "value")
        .coerce(std::bind(&b200_impl::coerce_rx_samp_rate, this, perif.ddc, dspno, std::placeholders::_1))
        .subscribe(std::bind(&lambda_set_bool_prop, boost::weak_ptr<property_tree>(_tree), rx_dsp_path / "rate" / "set", true, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_rx_samp_rate, this, dspno, std::placeholders::_1))
    ;
    _tree->create<stream_cmd_t>(rx_dsp_path / "stream_cmd")
        .subscribe(std::bind(&rx_vita_core_3000::issue_stream_command, perif.framer, std::placeholders::_1));
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&rx_vita_core_3000::set_tick_rate, perif.framer, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_rx_dsp_tick_rate, this, std::placeholders::_1, perif.ddc, rx_dsp_path))
    ;
    ...
    _tree->access<double>(tx_dsp_path / "rate" / "value")
        .coerce(std::bind(&b200_impl::coerce_tx_samp_rate, this, perif.duc, dspno, std::placeholders::_1))
        .subscribe(std::bind(&lambda_set_bool_prop, boost::weak_ptr<property_tree>(_tree), tx_dsp_path / "rate" / "set", true, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_tx_samp_rate, this, dspno, std::placeholders::_1))
    ;
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&tx_vita_core_3000::set_tick_rate, perif.deframer, std::placeholders::_1))
        .subscribe(std::bind(&b200_impl::update_tx_dsp_tick_rate, this, std::placeholders::_1, perif.duc, tx_dsp_path))
    ;
    ...
    BOOST_FOREACH(direction_t dir, dirs) {
        ...
        _tree->create<sensor_value_t>(rf_fe_path / "sensors" / "lo_locked")
            .publish(std::bind(&b200_impl::get_fe_pll_locked, this, dir == TX_DIRECTION))
        ;
        _tree->access<double>(rf_fe_path / "freq" / "value")
            .subscribe(std::bind(&b200_impl::update_bandsel, this, key, std::placeholders::_1))
        ;
        if (dir == RX_DIRECTION)
        {
            ...
            _tree->create<std::string>(rf_fe_path / "antenna" / "value")
                .subscribe(std::bind(&b200_impl::update_antenna_sel, this, dspno, std::placeholders::_1))
                .set("RX2")
            ;
