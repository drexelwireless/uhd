...
b100_impl::b100_impl(const device_addr_t &device_addr){
    ...
    _tree->create<std::string>(mb_path / "load_eeprom")
        .subscribe(std::bind(&fx2_ctrl::usrp_load_eeprom, _fx2_ctrl, std::placeholders::_1));
    ...
    _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
        .set(mb_eeprom)
        .subscribe(std::bind(&b100_impl::set_mb_eeprom, this, std::placeholders::_1));
    ...
    _tree->create<double>(mb_path / "tick_rate")
        .publish(std::bind(&b100_clock_ctrl::get_fpga_clock_rate, _clock_ctrl))
        .subscribe(std::bind(&fifo_ctrl_excelsior::set_tick_rate, _fifo_ctrl, std::placeholders::_1))
        .subscribe(std::bind(&b100_impl::update_tick_rate, this, std::placeholders::_1));

    //subscribe the command time while we are at it
    _tree->create<time_spec_t>(mb_path / "time/cmd")
        .subscribe(std::bind(&fifo_ctrl_excelsior::set_time, _fifo_ctrl, std::placeholders::_1));
    ...
    _tree->create<double>(rx_codec_path / "gains/pga/value")
        .coerce(std::bind(&b100_impl::update_rx_codec_gain, this, std::placeholders::_1))
        .set(0.0);
    _tree->create<std::string>(tx_codec_path / "name").set("ad9522");
    _tree->create<meta_range_t>(tx_codec_path / "gains/pga/range").set(b100_codec_ctrl::tx_pga_gain_range);
    _tree->create<double>(tx_codec_path / "gains/pga/value")
        .subscribe(std::bind(&b100_codec_ctrl::set_tx_pga_gain, _codec_ctrl, std::placeholders::_1))
        .publish(std::bind(&b100_codec_ctrl::get_tx_pga_gain, _codec_ctrl))
        .set(0.0);
    ...
    _tree->create<sensor_value_t>(mb_path / "sensors/ref_locked")
        .publish(std::bind(&b100_impl::get_ref_locked, this));
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
        .subscribe(std::bind(&b100_impl::update_rx_subdev_spec, this, std::placeholders::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
        .subscribe(std::bind(&b100_impl::update_tx_subdev_spec, this, std::placeholders::_1));

    const fs_path rx_fe_path = mb_path / "rx_frontends" / "A";
    const fs_path tx_fe_path = mb_path / "tx_frontends" / "A";

    _tree->create<std::complex<double> >(rx_fe_path / "dc_offset" / "value")
        .coerce(std::bind(&rx_frontend_core_200::set_dc_offset, _rx_fe, std::placeholders::_1))
        .set(std::complex<double>(0.0, 0.0));
    _tree->create<bool>(rx_fe_path / "dc_offset" / "enable")
        .subscribe(std::bind(&rx_frontend_core_200::set_dc_offset_auto, _rx_fe, std::placeholders::_1))
        .set(true);
    _tree->create<std::complex<double> >(rx_fe_path / "iq_balance" / "value")
        .subscribe(std::bind(&rx_frontend_core_200::set_iq_balance, _rx_fe, std::placeholders::_1))
        .set(std::complex<double>(0.0, 0.0));
    _tree->create<std::complex<double> >(tx_fe_path / "dc_offset" / "value")
        .coerce(std::bind(&tx_frontend_core_200::set_dc_offset, _tx_fe, std::placeholders::_1))
        .set(std::complex<double>(0.0, 0.0));
    _tree->create<std::complex<double> >(tx_fe_path / "iq_balance" / "value")
        .subscribe(std::bind(&tx_frontend_core_200::set_iq_balance, _tx_fe, std::placeholders::_1))
        .set(std::complex<double>(0.0, 0.0));
    ...
    for (size_t dspno = 0; dspno < num_rx_dsps; dspno++)
    {
        ...
        _tree->access<double>(mb_path / "tick_rate")
            .subscribe(std::bind(&rx_dsp_core_200::set_tick_rate, _rx_dsps[dspno], std::placeholders::_1));
        fs_path rx_dsp_path = mb_path / str(boost::format("rx_dsps/%u") % dspno);
        _tree->create<meta_range_t>(rx_dsp_path / "rate/range")
            .publish(std::bind(&rx_dsp_core_200::get_host_rates, _rx_dsps[dspno]));
        _tree->create<double>(rx_dsp_path / "rate/value")
            .set(1e6) //some default
            .coerce(std::bind(&rx_dsp_core_200::set_host_rate, _rx_dsps[dspno], std::placeholders::_1))
            .subscribe(std::bind(&b100_impl::update_rx_samp_rate, this, dspno, std::placeholders::_1));
        _tree->create<double>(rx_dsp_path / "freq/value")
            .coerce(std::bind(&rx_dsp_core_200::set_freq, _rx_dsps[dspno], std::placeholders::_1));
        _tree->create<meta_range_t>(rx_dsp_path / "freq/range")
            .publish(std::bind(&rx_dsp_core_200::get_freq_range, _rx_dsps[dspno]));
        _tree->create<stream_cmd_t>(rx_dsp_path / "stream_cmd")
            .subscribe(std::bind(&rx_dsp_core_200::issue_stream_command, _rx_dsps[dspno], std::placeholders::_1));
    }
    ...
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&tx_dsp_core_200::set_tick_rate, _tx_dsp, std::placeholders::_1));
    _tree->create<meta_range_t>(mb_path / "tx_dsps/0/rate/range")
        .publish(std::bind(&tx_dsp_core_200::get_host_rates, _tx_dsp));
    _tree->create<double>(mb_path / "tx_dsps/0/rate/value")
        .set(1e6) //some default
        .coerce(std::bind(&tx_dsp_core_200::set_host_rate, _tx_dsp, std::placeholders::_1))
        .subscribe(std::bind(&b100_impl::update_tx_samp_rate, this, 0, std::placeholders::_1));
    _tree->create<double>(mb_path / "tx_dsps/0/freq/value")
        .coerce(std::bind(&tx_dsp_core_200::set_freq, _tx_dsp, std::placeholders::_1));
    _tree->create<meta_range_t>(mb_path / "tx_dsps/0/freq/range")
        .publish(std::bind(&tx_dsp_core_200::get_freq_range, _tx_dsp));
    ...
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&time64_core_200::set_tick_rate, _time64, std::placeholders::_1));
    _tree->create<time_spec_t>(mb_path / "time/now")
        .publish(std::bind(&time64_core_200::get_time_now, _time64))
        .subscribe(std::bind(&time64_core_200::set_time_now, _time64, std::placeholders::_1));
    _tree->create<time_spec_t>(mb_path / "time/pps")
        .publish(std::bind(&time64_core_200::get_time_last_pps, _time64))
        .subscribe(std::bind(&time64_core_200::set_time_next_pps, _time64, std::placeholders::_1));
    //setup time source props
    _tree->create<std::string>(mb_path / "time_source/value")
        .subscribe(std::bind(&time64_core_200::set_time_source, _time64, std::placeholders::_1));
    _tree->create<std::vector<std::string> >(mb_path / "time_source/options")
        .publish(std::bind(&time64_core_200::get_time_sources, _time64));
    //setup reference source props
    _tree->create<std::string>(mb_path / "clock_source/value")
        .subscribe(std::bind(&b100_impl::update_clock_source, this, std::placeholders::_1));
    ...
    _tree->create<user_settings_core_200::user_reg_t>(mb_path / "user/regs")
        .subscribe(std::bind(&user_settings_core_200::set_reg, _user, std::placeholders::_1));
    ...
    _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/rx_eeprom")
        .set(rx_db_eeprom)
        .subscribe(std::bind(&b100_impl::set_db_eeprom, this, "rx", std::placeholders::_1));
    _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/tx_eeprom")
        .set(tx_db_eeprom)
        .subscribe(std::bind(&b100_impl::set_db_eeprom, this, "tx", std::placeholders::_1));
    _tree->create<dboard_eeprom_t>(mb_path / "dboards/A/gdb_eeprom")
        .set(gdb_eeprom)
        .subscribe(std::bind(&b100_impl::set_db_eeprom, this, "gdb", std::placeholders::_1));
    ...
    BOOST_FOREACH(const std::string &name, _tree->list(db_tx_fe_path)){
        _tree->access<double>(db_tx_fe_path / name / "freq" / "value")
            .subscribe(std::bind(&b100_impl::set_tx_fe_corrections, this, std::placeholders::_1));
    }
    const fs_path db_rx_fe_path = mb_path / "dboards" / "A" / "rx_frontends";
    BOOST_FOREACH(const std::string &name, _tree->list(db_rx_fe_path)){
        _tree->access<double>(db_rx_fe_path / name / "freq" / "value")
            .subscribe(std::bind(&b100_impl::set_rx_fe_corrections, this, std::placeholders::_1));
    }
    ...
    _tree->access<double>(mb_path / "tick_rate") //now subscribe the clock rate setter
        .subscribe(std::bind(&b100_clock_ctrl::set_fpga_clock_rate, _clock_ctrl, std::placeholders::_1));
