...
public:
    ...
    void populate_subtree(property_tree::sptr subtree)
    {
        subtree->create<meta_range_t>("rate/range")
            .publish(std::bind(&rx_dsp_core_3000::get_host_rates, this))
        ;
        subtree->create<double>("rate/value")
            .set(DEFAULT_RATE)
            .coerce(std::bind(&rx_dsp_core_3000::set_host_rate, this, std::placeholders::_1))
        ;
        subtree->create<double>("freq/value")
            .set(DEFAULT_CORDIC_FREQ)
            .coerce(std::bind(&rx_dsp_core_3000::set_freq, this, std::placeholders::_1))
        ;
        subtree->create<meta_range_t>("freq/range")
            .publish(std::bind(&rx_dsp_core_3000::get_freq_range, this))
        ;
    }
