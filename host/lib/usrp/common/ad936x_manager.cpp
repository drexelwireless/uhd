  ...
  public:
    ...
    void populate_frontend_subtree(uhd::property_tree::sptr subtree, const std::string &key, uhd::direction_t dir)
    {
        ...
        subtree->create<sensor_value_t>("sensors/temp")
            .publish(std::bind(&ad9361_ctrl::get_temperature, _codec_ctrl))
        ;
        if (dir == RX_DIRECTION) {
            subtree->create<sensor_value_t>("sensors/rssi")
                .publish(std::bind(&ad9361_ctrl::get_rssi, _codec_ctrl, key))
            ;
        }
        ...
        BOOST_FOREACH(const std::string &name, ad9361_ctrl::get_gain_names(key))
        {
            ...
            subtree->create<double>(uhd::fs_path("gains") / name / "value")
                ...
                .coerce(std::bind(&ad9361_ctrl::set_gain, _codec_ctrl, key, std::placeholders::_1))
            ;
        }
        ...
        subtree->create<double>("bandwidth/value")
            ...
            .coerce(std::bind(&ad9361_ctrl::set_bw_filter, _codec_ctrl, key, std::placeholders::_1))
        ;
        subtree->create<meta_range_t>("bandwidth/range")
            .publish(std::bind(&ad9361_ctrl::get_bw_filter_range, key))
        ;

        // LO Tuning
        subtree->create<meta_range_t>("freq/range")
            .publish(std::bind(&ad9361_ctrl::get_rf_freq_range))
        ;
        subtree->create<double>("freq/value")
            .publish(std::bind(&ad9361_ctrl::get_freq, _codec_ctrl, key))
            .coerce(std::bind(&ad9361_ctrl::tune, _codec_ctrl, key, std::placeholders::_1))
        ;
        ...
        if(dir == RX_DIRECTION)
        {
            subtree->create<bool>("dc_offset/enable" )
                ...
                .subscribe(std::bind(&ad9361_ctrl::set_dc_offset_auto, _codec_ctrl, key, std::placeholders::_1))
            ;
            subtree->create<bool>("iq_balance/enable" )
                .set(ad936x_manager::DEFAULT_AUTO_IQ_BALANCE)
                .subscribe(std::bind(&ad9361_ctrl::set_iq_balance_auto, _codec_ctrl, key, std::placeholders::_1))
            ;
            ...
            subtree->create<bool>("gain/agc/enable")
                ...
                .subscribe(std::bind((&ad9361_ctrl::set_agc), _codec_ctrl, key, std::placeholders::_1))
            ;
            subtree->create<std::string>("gain/agc/mode/value")
                .subscribe(std::bind((&ad9361_ctrl::set_agc_mode), _codec_ctrl, key, std::placeholders::_1)).set(mode_strings.front())
            ;
        ...
        BOOST_FOREACH(const std::string &filter_name, _codec_ctrl->get_filter_names(key)) {
            subtree->create<filter_info_base::sptr>(uhd::fs_path("filters") / filter_name / "value" )
                .publish(std::bind(&ad9361_ctrl::get_filter, _codec_ctrl, key, filter_name))
                .subscribe(std::bind(&ad9361_ctrl::set_filter, _codec_ctrl, key, filter_name, std::placeholders::_1));
        }
    }
