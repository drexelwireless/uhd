...
void x300_impl::setup_mb(const size_t mb_i, const uhd::device_addr_t &dev_addr)
{
    ...
    mb.claimer_task = uhd::task::make(std::bind(&x300_impl::claimer_loop, this, mb.zpu_ctrl));
    ...
    */
    ...
    _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
        .set(mb_eeprom)
        .subscribe(std::bind(&x300_impl::set_mb_eeprom, this, mb.zpu_i2c, std::placeholders::_1));
    ...
    _tree->create<double>(mb_path / "tick_rate")
        .publish(std::bind(&x300_clock_ctrl::get_master_clock_rate, mb.clock));
        ...
        if (mb.gps and mb.gps->gps_detected())
        {
            BOOST_FOREACH(const std::string &name, mb.gps->get_sensors())
            {
                _tree->create<sensor_value_t>(mb_path / "sensors" / name)
                    .publish(std::bind(&gps_ctrl::get_sensor, mb.gps, name));
            }
        }
        else
        {
    ...
    BOOST_FOREACH(const gpio_attr_map_t::value_type attr, gpio_attr_map)
    {
        _tree->create<boost::uint32_t>(mb_path / "gpio" / "FP0" / attr.second)
            .set(0)
            .subscribe(std::bind(&x300_impl::set_fp_gpio, this, mb.fp_gpio, attr.first, std::placeholders::_1));
    }
    _tree->create<boost::uint32_t>(mb_path / "gpio" / "FP0" / "READBACK")
        .publish(std::bind(&x300_impl::get_fp_gpio, this, mb.fp_gpio));

    ////////////////////////////////////////////////////////////////////
    // register the time keepers - only one can be the highlander
    ////////////////////////////////////////////////////////////////////
    _tree->create<time_spec_t>(mb_path / "time" / "now")
        .publish(std::bind(&time_core_3000::get_time_now, mb.radio_perifs[0].time64))
        .subscribe(std::bind(&x300_impl::sync_times, this, mb, std::placeholders::_1))
        .set(0.0);
    _tree->create<time_spec_t>(mb_path / "time" / "pps")
        .publish(std::bind(&time_core_3000::get_time_last_pps, mb.radio_perifs[0].time64))
        .subscribe(std::bind(&time_core_3000::set_time_next_pps, mb.radio_perifs[0].time64, std::placeholders::_1))
        .subscribe(std::bind(&time_core_3000::set_time_next_pps, mb.radio_perifs[1].time64, std::placeholders::_1));
    ...
    _tree->create<std::string>(mb_path / "time_source" / "value")
        .set("internal")
        .subscribe(std::bind(&x300_impl::update_time_source, this, boost::ref(mb), std::placeholders::_1));
    ...
    _tree->create<bool>(mb_path / "time_source" / "output")
        .subscribe(std::bind(&x300_impl::set_time_source_out, this, boost::ref(mb), std::placeholders::_1))
        .set(true);
    ...
    _tree->create<std::string>(mb_path / "clock_source" / "value")
        .set("internal")
        .subscribe(std::bind(&x300_impl::update_clock_source, this, boost::ref(mb), std::placeholders::_1));
    ...
    _tree->create<bool>(mb_path / "clock_source" / "output")
        .subscribe(std::bind(&x300_clock_ctrl::set_ref_out, mb.clock, std::placeholders::_1));

    //initialize tick rate (must be done before setting time)
    _tree->access<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&x300_impl::set_tick_rate, this, boost::ref(mb), std::placeholders::_1))
        .subscribe(std::bind(&x300_impl::update_tick_rate, this, boost::ref(mb), std::placeholders::_1))
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
        .subscribe(std::bind(&x300_impl::update_subdev_spec, this, "rx", mb_i, std::placeholders::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
        .subscribe(std::bind(&x300_impl::update_subdev_spec, this, "tx", mb_i, std::placeholders::_1));

    ////////////////////////////////////////////////////////////////////
    // and do the misc mboard sensors
    ////////////////////////////////////////////////////////////////////
    _tree->create<sensor_value_t>(mb_path / "sensors" / "ref_locked")
        .publish(std::bind(&x300_impl::get_ref_locked, this, mb));
...
void x300_impl::setup_radio(const size_t mb_i, const std::string &slot_name, const uhd::device_addr_t &dev_addr)
{
    ...
    //    .subscribe(std::bind(&x300_adc_ctrl::set_gain, perif.adc, std::placeholders::_1)).set(0);
    ...
    _tree->access<double>(rx_dsp_path / "rate" / "value")
        .subscribe(std::bind(&x300_impl::update_rx_samp_rate, this, boost::ref(mb), radio_index, std::placeholders::_1))
    ;
    ...
    _tree->access<double>(tx_dsp_path / "rate" / "value")
        .subscribe(std::bind(&x300_impl::update_tx_samp_rate, this, boost::ref(mb), radio_index, std::placeholders::_1))
    ;
    ...
    _tree->create<dboard_eeprom_t>(db_path / "rx_eeprom")
        ...
        .subscribe(std::bind(&x300_impl::set_db_eeprom, this, mb.zpu_i2c, (0x50 | X300_DB0_RX_EEPROM | j), std::placeholders::_1));
    _tree->create<dboard_eeprom_t>(db_path / "tx_eeprom")
        .set(mb.db_eeproms[X300_DB0_TX_EEPROM | j])
        .subscribe(std::bind(&x300_impl::set_db_eeprom, this, mb.zpu_i2c, (0x50 | X300_DB0_TX_EEPROM | j), std::placeholders::_1));
    _tree->create<dboard_eeprom_t>(db_path / "gdb_eeprom")
        .set(mb.db_eeproms[X300_DB0_GDB_EEPROM | j])
        .subscribe(std::bind(&x300_impl::set_db_eeprom, this, mb.zpu_i2c, (0x50 | X300_DB0_GDB_EEPROM | j), std::placeholders::_1));
    ...
    _tree->access<std::string>(db_path / "rx_frontends" / fe_name / "antenna" / "value")
        .subscribe(std::bind(&x300_impl::update_atr_leds, this, mb.radio_perifs[radio_index].leds, std::placeholders::_1));
    ...
    BOOST_FOREACH(const std::string &name, _tree->list(db_tx_fe_path)) {
        _tree->access<double>(db_tx_fe_path / name / "freq" / "value")
            .subscribe(std::bind(&x300_impl::set_tx_fe_corrections, this, mb_path, slot_name, std::placeholders::_1));
    }
    const fs_path db_rx_fe_path = db_path / "rx_frontends";
    BOOST_FOREACH(const std::string &name, _tree->list(db_rx_fe_path)) {
        _tree->access<double>(db_rx_fe_path / name / "freq" / "value")
            .subscribe(std::bind(&x300_impl::set_rx_fe_corrections, this, mb_path, slot_name, std::placeholders::_1));
    }
}
