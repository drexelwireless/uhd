...
usrp1_impl::usrp1_impl(const device_addr_t &device_addr){
    ...
    _tree->create<std::pair<boost::uint8_t, boost::uint32_t> >(mb_path / "user" / "regs")
        .subscribe(std::bind(&usrp1_impl::set_reg, this, std::placeholders::_1));
    ...
    _tree->create<mboard_eeprom_t>(mb_path / "eeprom")
        .set(mb_eeprom)
        .subscribe(std::bind(&usrp1_impl::set_mb_eeprom, this, std::placeholders::_1));
    ...
    _tree->create<double>(mb_path / "tick_rate")
        .subscribe(std::bind(&usrp1_impl::update_tick_rate, this, std::placeholders::_1))
    ...
    BOOST_FOREACH(const std::string &db, _dbc.keys()){
        ...
        _tree->create<double>(rx_codec_path / "gains/pga/value")
            .coerce(std::bind(&usrp1_impl::update_rx_codec_gain, this, db, std::placeholders::_1))
            .set(0.0);
        _tree->create<std::string>(tx_codec_path / "name").set("ad9522");
        _tree->create<meta_range_t>(tx_codec_path / "gains/pga/range").set(usrp1_codec_ctrl::tx_pga_gain_range);
        _tree->create<double>(tx_codec_path / "gains/pga/value")
            .subscribe(std::bind(&usrp1_codec_ctrl::set_tx_pga_gain, _dbc[db].codec, std::placeholders::_1))
            .publish(std::bind(&usrp1_codec_ctrl::get_tx_pga_gain, _dbc[db].codec))
            .set(0.0);
    }
    ...
    _tree->create<subdev_spec_t>(mb_path / "rx_subdev_spec")
        .set(subdev_spec_t())
        .subscribe(std::bind(&usrp1_impl::update_rx_subdev_spec, this, std::placeholders::_1));
    _tree->create<subdev_spec_t>(mb_path / "tx_subdev_spec")
        .set(subdev_spec_t())
        .subscribe(std::bind(&usrp1_impl::update_tx_subdev_spec, this, std::placeholders::_1));

    BOOST_FOREACH(const std::string &db, _dbc.keys()){
        const fs_path rx_fe_path = mb_path / "rx_frontends" / db;
        _tree->create<std::complex<double> >(rx_fe_path / "dc_offset" / "value")
            .coerce(std::bind(&usrp1_impl::set_rx_dc_offset, this, db, std::placeholders::_1))
            .set(std::complex<double>(0.0, 0.0));
        _tree->create<bool>(rx_fe_path / "dc_offset" / "enable")
            .subscribe(std::bind(&usrp1_impl::set_enb_rx_dc_offset, this, db, std::placeholders::_1))
            .set(true);
    }
    ...
    for (size_t dspno = 0; dspno < get_num_ddcs(); dspno++){
        ...
        _tree->create<meta_range_t>(rx_dsp_path / "rate/range")
            .publish(std::bind(&usrp1_impl::get_rx_dsp_host_rates, this));
        _tree->create<double>(rx_dsp_path / "rate/value")
            .set(1e6) //some default rate
            .coerce(std::bind(&usrp1_impl::update_rx_samp_rate, this, dspno, std::placeholders::_1));
        _tree->create<double>(rx_dsp_path / "freq/value")
            .coerce(std::bind(&usrp1_impl::update_rx_dsp_freq, this, dspno, std::placeholders::_1));
        _tree->create<meta_range_t>(rx_dsp_path / "freq/range")
            .publish(std::bind(&usrp1_impl::get_rx_dsp_freq_range, this));
        ...
        if (dspno == 0){
            ...
            _tree->access<stream_cmd_t>(rx_dsp_path / "stream_cmd")
                .subscribe(std::bind(&soft_time_ctrl::issue_stream_cmd, _soft_time_ctrl, std::placeholders::_1));
        }
    }
    ...
    for (size_t dspno = 0; dspno < get_num_ducs(); dspno++){
        ...
        _tree->create<meta_range_t>(tx_dsp_path / "rate/range")
            .publish(std::bind(&usrp1_impl::get_tx_dsp_host_rates, this));
        _tree->create<double>(tx_dsp_path / "rate/value")
            .set(1e6) //some default rate
            .coerce(std::bind(&usrp1_impl::update_tx_samp_rate, this, dspno, std::placeholders::_1));
        _tree->create<double>(tx_dsp_path / "freq/value")
            .coerce(std::bind(&usrp1_impl::update_tx_dsp_freq, this, dspno, std::placeholders::_1));
        _tree->create<meta_range_t>(tx_dsp_path / "freq/range")
            .publish(std::bind(&usrp1_impl::get_tx_dsp_freq_range, this));
    }
    ...
    _tree->create<time_spec_t>(mb_path / "time/now")
        .publish(std::bind(&soft_time_ctrl::get_time, _soft_time_ctrl))
        .subscribe(std::bind(&soft_time_ctrl::set_time, _soft_time_ctrl, std::placeholders::_1));
    ...
    BOOST_FOREACH(const std::string &db, _dbc.keys()){
        ...
        _tree->create<dboard_eeprom_t>(mb_path / "dboards" / db/ "rx_eeprom")
            .set(rx_db_eeprom)
            .subscribe(std::bind(&usrp1_impl::set_db_eeprom, this, db, "rx", std::placeholders::_1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards" / db/ "tx_eeprom")
            .set(tx_db_eeprom)
            .subscribe(std::bind(&usrp1_impl::set_db_eeprom, this, db, "tx", std::placeholders::_1));
        _tree->create<dboard_eeprom_t>(mb_path / "dboards" / db/ "gdb_eeprom")
            .set(gdb_eeprom)
            .subscribe(std::bind(&usrp1_impl::set_db_eeprom, this, db, "gdb", std::placeholders::_1));
