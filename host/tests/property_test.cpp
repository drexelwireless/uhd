...
BOOST_AUTO_TEST_CASE(test_prop_with_subscriber){
    ...
    setter_type setter;
    prop.subscribe(std::bind(&setter_type::doit, &setter, std::placeholders::_1));
...
BOOST_AUTO_TEST_CASE(test_prop_with_publisher){
    ...
    BOOST_CHECK(prop.empty());
    getter_type getter;
    prop.publish(std::bind(&getter_type::doit, &getter));
    BOOST_CHECK(not prop.empty());
...
BOOST_AUTO_TEST_CASE(test_prop_with_coercion){
    ...
    prop.coerce(std::bind(&coercer_type::doit, &coercer, std::placeholders::_1));
